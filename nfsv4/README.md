...
This is the version-specific guide for the NFSv4 lab in this repository.
...
This setup creates:
1. Ubuntu 18.04-based NFSv4 server container with pseudo-root `root_squash`
2. Ubuntu 18.04-based NFSv4 server container with pseudo-root `no_root_squash`
3. Ubuntu 18.04-based NFSv4 server container for `all_squash` + ubuntu anon mapping (`65534:65534`)
4. Ubuntu 18.04-based NFSv4 server container for `all_squash` + redhat anon mapping (`65533:65533`)
5. Ubuntu 18.04-based NFSv4 server container for `all_squash` + windows anon mapping (`65532:65532`)
...
## Files
- `nfsv4/nfs-server/Dockerfile`
- `nfsv4/nfs-server/entrypoint.sh`
- `nfsv4/nfs-client/Dockerfile`
- `nfsv4/nfs-client/entrypoint.sh`
- `docker-compose.yml`
...
```bash
docker exec -it nfs-client python3 /usr/local/bin/validate_readme_matrix.py --readme /workspace/nfsv4/README.md --mount-base /mnt/nfs
```

Quick dry run (first 10 rows):
```bash
docker exec -it nfs-client python3 /usr/local/bin/validate_readme_matrix.py --readme /workspace/nfsv4/README.md --mount-base /mnt/nfs --max-rows 10
```
