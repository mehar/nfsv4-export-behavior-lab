# NFSv3 Export Behavior Lab

This is the version-specific guide for the NFSv3 lab in this repository.

## Why this project exists

NFSv3 export behavior can be hard to reason about when ownership, mode bits, and squash policies interact.
...
- Client uses NFSv3 direct export paths via `NFS_EXPORT_BASE=/exports`
- Default server labels:
  - `root_squash` -> `nfsv3-nfs-server-root-squash`
  - `all_squash_ubuntu` -> `nfsv3-nfs-server-all-squash-ubuntu`
  - `all_squash_redhat` -> `nfsv3-nfs-server-all-squash-redhat`
  - `all_squash_windows` -> `nfsv3-nfs-server-all-squash-windows`
  - `no_root_squash` -> `nfsv3-nfs-server-no-root-squash`
- Client mount default options: `vers=3,soft,timeo=50,retrans=2` (override with `MOUNT_OPTS`)

## Why this uses a 5-server model

We split this into 5 NFS servers because of how NFSv3 export behavior is applied in this containerized test setup.
...
## Files
- `nfsv3/nfs-server/Dockerfile`
- `nfsv3/nfs-server/entrypoint.sh`
- `nfsv3/nfs-client/Dockerfile`
- `nfsv3/nfs-client/entrypoint.sh`
- `docker-compose.yml`
...
```bash
docker exec -it nfsv3-nfs-server-root-squash bash
docker exec -it nfsv3-nfs-server-all-squash-ubuntu bash
docker exec -it nfsv3-nfs-server-all-squash-redhat bash
docker exec -it nfsv3-nfs-server-all-squash-windows bash
docker exec -it nfsv3-nfs-server-no-root-squash bash
docker exec -it nfsv3-nfs-client bash
```

## Verify server exports
```bash
docker exec -it nfsv3-nfs-server-root-squash ls -la /exports
docker exec -it nfsv3-nfs-server-root-squash cat /etc/exports
docker exec -it nfsv3-nfs-server-all-squash-ubuntu ls -la /exports
docker exec -it nfsv3-nfs-server-all-squash-ubuntu cat /etc/exports
docker exec -it nfsv3-nfs-server-all-squash-redhat ls -la /exports
docker exec -it nfsv3-nfs-server-all-squash-redhat cat /etc/exports
docker exec -it nfsv3-nfs-server-all-squash-windows ls -la /exports
docker exec -it nfsv3-nfs-server-all-squash-windows cat /etc/exports
docker exec -it nfsv3-nfs-server-no-root-squash ls -la /exports
docker exec -it nfsv3-nfs-server-no-root-squash cat /etc/exports
```

## Verify client mounts
```bash
docker exec -it nfsv3-nfs-client mount | grep /mnt/nfs
docker exec -it nfsv3-nfs-client ls -la /mnt/nfs
docker exec -it nfsv3-nfs-client ls -la /mnt/nfs/owner_root_perm_755_squash_root_squash_anon_ubuntu
docker exec -it nfsv3-nfs-client ls -la /mnt/nfs/owner_root_perm_755_squash_no_squash
```

## Validate README Matrix From Client
```bash
docker exec -it nfsv3-nfs-client python3 /usr/local/bin/validate_readme_matrix.py --readme /workspace/nfsv3/README.md --mount-base /mnt/nfs
```

Quick dry run (first 10 rows):
```bash
docker exec -it nfsv3-nfs-client python3 /usr/local/bin/validate_readme_matrix.py --readme /workspace/nfsv3/README.md --mount-base /mnt/nfs --max-rows 10
```
...
## Write/read test
```bash
docker exec -it nfsv3-nfs-client sh -lc 'echo hello-from-client > /mnt/nfs/owner_root_perm_777_squash_no_squash/client-test.txt'
docker exec -it nfsv3-nfs-server-no-root-squash sh -lc 'cat /exports/owner_root_perm_777_squash_no_squash/client-test.txt'
```
